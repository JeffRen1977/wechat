...
- **OpenAI images**: When asked to generate cover or figures, run via **bash**: `./scripts/run-image-generation.sh wechat_factory/04_output/YYYY-MM-DD/<PREFIX>_article.md`. Produces **1 cover + 2 content images** per article; prompts use **title + H2 section text** and **domain-locked palettes** (EDU/MED/FIN/INBOX) - see `scripts/image-generation.py`. Draft articles with **concrete nouns under each H2** for better fig relevance. This wrapper sources `~/.openai-env` and `~/.wechat-env` then runs the image script. Ensure one of those env files contains `export OPENAI_API_KEY=...` (Agent does not load `~/.bashrc`). See IMPLEMENTATION.md 4.5.
